...
object HAdjacencies {
  ...
  def apply[V, @sp(Double) A: Order, M](hPolytope: HPolytope[V, A], vPolytope: VPolytope[V, A])(implicit alg: AlgMVF[M, V, A]): HAdjacencies[V, A] = apply(hPolytope, hPolytope.vertexSets(vPolytope))
}
