...
object H1999Format extends CddFileFormat {
  ...
  def readPolytope(s: String): HRepr = {
    ...
    val data = res._2._4
    val A = qalg.mutable.QMatrix.fill(ineq, if (ineq == 0) 0 else d)(Rational.zero)
    val b = qalg.mutable.QVector.fill(ineq)(Rational.zero)
    val Aeq = qalg.mutable.QMatrix.fill(eq, if (eq == 0) 0 else d)(Rational.zero)
    val beq = qalg.mutable.QVector.fill(eq)(Rational.zero)
    var i = 0
    var ieq = 0
...
object V1999Format extends CddFileFormat {
  ...
  def readPolytope(s: String): VRepr = {
    ...
    val E = qalg.mutable.QMatrix.fill(nE, if (nE == 0) 0 else d)(Rational.zero)
    val R = qalg.mutable.QMatrix.fill(nR, if (nR == 0) 0 else d)(Rational.zero)
    var kE = 0
    var kR = 0
