...
object POIFormat extends PortaFileFormat {
  ...
  def writePolytope(p: VRepr): String = {
    ...
    str ++= "DIM = " + p.dim + "\n"
    def printMatrix(m: qalg.immutable.QMatrix) = 
  ...
  def readPolytope(s: String) = {
    ...
    val E = qalg.mutable.QMatrix.fill(e.size, if (e.size == 0) 0 else d)(Rational.zero)
    val R = qalg.mutable.QMatrix.fill(r.size, if (r.size == 0) 0 else d)(Rational.zero)
...
object IEQFormat extends PortaFileFormat {
  ...
  case class IEQData(d: Int,
    v: Option[qalg.mutable.QVector] = None,
  ...
  def content = dimension >> ( (d: Int) => (rep(ieq_element(d)) <~ end) ^^ {
    list => list.foldLeft(IEQData(d)) {
      case (dt, x: Valid) => dt.copy(v = Some(qalg.mutable.QVector(x.x:_*)))
  ...
  def writePolytope(p: HRepr) = {
    val str = new StringBuilder()
    str ++= "DIM = " + p.dim + "\n"
    def printLinear(a: qalg.immutable.QMatrix, b: qalg.immutable.QVector, op: String) = {
  ...
  def readPolytope(s: String) = {
    ...
    val a = qalg.mutable.QMatrix.fill(ineqs.size, if (ineqs.size == 0) 0 else d)(Rational.zero)
    val b = qalg.mutable.QVector.fill(ineqs.size)(Rational.zero)
    val aeq = qalg.mutable.QMatrix.fill(eqs.size, if (eqs.size == 0) 0 else d)(Rational.zero)
    val beq = qalg.mutable.QVector.fill(eqs.size)(Rational.zero)
