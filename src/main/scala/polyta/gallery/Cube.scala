...
object Cube {
  ...
  def inV[A](dim: Int)(implicit A: LinAlg[A]): VPolytopeCombSym[A] = {
    ...
    VPolytope[A](dim, vertices, Seq.empty)
  }

  def inH[A](dim: Int)(implicit A: LinAlg[A]): HPolytopeCombSym[A] = {
    import A.{fieldA, orderA, IVec, IMat}
    ...
    HPolytope[A](dim, (0 until dim).flatMap(k => Seq(constraint1(k), constraint2(k))), Seq.empty)
  }
